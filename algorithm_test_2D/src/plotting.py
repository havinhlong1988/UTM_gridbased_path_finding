...
def plot_path_report(
    ...
    Notes:
        ...
        - No-fly nodes are still overlaid explicitly as red squares.
    """
...
def _write_interval_indexed_cpt(
    ...
    # Make sure exact no-fly value, e.g. 10.0, can get its own red class.
    ...
    # Set F to no-fly color so exact/high no-fly values stay red.
...
def plot_costmap_layer(
    ...
    if plot_no_fly_overlay:
        try:
            ...
            if not nofly.empty:
                fig.plot(x=nofly["x"], y=nofly["y"], style=f"s{max(marker_size / 18.0, 0.06):.3f}c", fill="red@70", pen="0.15p,red", label="No-fly")
        except Exception:
            pass
