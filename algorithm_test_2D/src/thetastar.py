...
def _bresenham_segment_indices(model, cache: GridCache, idx1: int, idx2: int) -> list[int] | None:
    """Return model indices crossed by a Bresenham LOS segment.

    Safety rule
    -----------
    Every returned model index must be traversable later in _line_of_sight().
    If a Bresenham cell cannot be mapped back to a model node, this function
    returns None and the shortcut is rejected.  This prevents hidden gaps in
    the model grid from being treated as flyable.

    Clearance
    ---------
    cache.bresenham_clearance_cells controls how conservative LOS is:
      0: check only cells exactly on the Bresenham line.
      1: also check the 8 neighboring cells around each line cell.

    For debugging Theta* smoothing, start with clearance 0.  If the shortcut
    looks too close to no-fly cells, increase to 1.
    """
    ...
    for gx, gy in line_cells:
        # Check the line cell plus optional clearance cells around it.
    ...
    # Keep exact endpoints in the returned list, even if clearance ordering made
    # them appear later.  This is useful for cost and debug output.
